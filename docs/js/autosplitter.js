_autosplitter = function () {
	var wrs = ["2.06", "2.90", "3.17", "2.35", "1.77", "2.71", "4.35", "4.03", "3.31", "4.56", "2.49", "1.97", "3.99", "4.38", "7.93"];
	...
	*/
	var moveToLevel = function (level) {
		window.game.dt(window.game.cd[level + 2]);
	}

	var isInSpeedrun = function () {
		var speedrunning_var = window.game.ez.find(obj => obj.name === "IsSpeedRunning");
	...
	*/
	var onSound = function (soundName) {
		...
		// In Greedy Mimic - the "damsel" sound effect only plays on the last character eaten
		if (soundName == "damsel" && state.in_level) {
